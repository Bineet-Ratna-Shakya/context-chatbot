...
with LangGraph for control flow, FastAPI for the API, FAISS for vector search, and
...
FAISS index. Embeddings are normalized so similarity scores are comparable across
queries.

At query time the user's question is embedded with the same model and compared
against the index. Chunks beyond a configurable L2 distance threshold are dropped,
...
standalone search query using earlier turns, retrieve runs the FAISS similarity
...
dependency management, builds the FAISS index from the committed corpus at build
